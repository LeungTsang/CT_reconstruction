...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        self.thetas = torch.linspace(0, np.pi-(np.pi/256), 256).cuda()
    ...
    def make_pos_embed(self, embed_dim, thetas):
        ...
        emb = torch.stack([emb_sin,emb_cos],dim=1).repeat(1,embed_dim//2)
        return emb
    ...
    def forward_loss(self, sinograms, reconstruction, mask):
    
        reconstruction = torch.nn.functional.interpolate(reconstruction, self.reconstruction_size, mode="bilinear")
        pred_sinograms = self.radon(reconstruction)
        loss = torch.nn.functional.mse_loss(pred_sinograms,sinograms)
        
        return loss

    def forward(self, sinograms, mask_ratio=0.25):
        latent, mask, ids_restore = self.forward_encoder(sinograms, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        reconstruction = self.unpatchify(pred)
        loss = self.forward_loss(sinograms,reconstruction, mask)
        return reconstruction, loss, mask


def mae_vit_base_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, embed_dim=512, depth=4, num_heads=4,
